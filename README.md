...
FinPay microservice — customer-service. Part of the [finpay-lab](https://github.com/finpay-lab) multi-repository
...
- Customer profile aggregate + idempotent onboarding (FP-2).
- AI-assisted customer explainer (`HttpLlmSynthesizer`) + OpenSearch event store for search/synthesis.
...
Images are built, `kind load`ed as `finpaylab/customer-service:fp9`, and rolled out via the
`finpay-services` Argo CD ApplicationSet. The API entrypoint is the **gateway**
(port 8080); this service is reached internally on port 8084.
